...
while [ $COMPRESSED ]
        do
                case $COMPRESSED in
                  ...
                  *) echo "Please enter a VALID answer compression [Y|N]" ;;
                esac
        done
...
while [ $LEVEL ]
        do
                case $LEVEL in
                   ""|0|y|Y|yes|YES|Yes) LEVEL=" LEVEL=0 ";LOG=full;break ;;
                    1|n|N|no|NO|No) LEVEL=" LEVEL=1 ";LOG=incr;break ;;  
		*) echo "Please enter a VALID answer [Y|N]" ;;
                esac
	done

RMANSCRIPT=${BKPLOC}/RMAN_${LOG}_${ORACLE_SID}.rman
RMANLOG=${BKPLOC}/rman_${LOG}_log.`date '+%a'`
...
echo "BACKUP ${COMPRESSED} INCREMENTAL $LEVEL FORMAT '${BKPLOC}/%d_%t_%s_%p' TAG='${LOG}'" >> ${RMANSCRIPT}
